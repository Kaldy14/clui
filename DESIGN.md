...
- Progressive disclosure: Collapsed nodes are overview labels, not summaries: show only type, status, a single-line title, expand/focus controls, and a compact agent-output action when real output exists. Reveal summaries, forms, questionnaires, todos, artifacts, and activity only after expansion. Show each semantic fact once; do not pair a large type icon with a redundant type label.
...
- Agent output: Opening live output is non-destructive and preserves graph position. A collapsed node with real agent output exposes a compact, labelled icon action; the action indicates when that node's inspector is open. The inspector renders the selected harness's native live event stream, distinguishes a running stream from completed history, and can be closed without stopping the agent.
