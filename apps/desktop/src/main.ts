...
import type { ContextMenuItem } from "@clui/contracts";
...
const APP_DISPLAY_NAME = isDevelopment ? "Clui (Dev)" : "Clui (Alpha)";
const APP_USER_MODEL_ID = "com.clui.app";
const USER_DATA_DIR_NAME = isDevelopment ? "clui-dev" : "clui";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "Clui (Dev)" : "Clui (Alpha)";
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { cluiCommitHash?: unknown };
    return normalizeCommitHash(parsed.cluiCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.CLUI_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Clui failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const disabledReason = getAutoUpdateDisabledReason({
    ...
    disabledByEnv: process.env.CLUI_DISABLE_AUTO_UPDATE === "1",
  });
  if (disabledReason) {
 ...
 * parentheses (e.g. `~/.config/Clui (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`clui`). If the legacy
...
function configureAutoUpdater(): void {
  ...
  const githubToken =
    process.env.CLUI_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() || process.env.GH_TOKEN?.trim() || "";
  if (githubToken) {
...
async function bootstrap(): Promise<void> {
  ...
  process.env.CLUI_DESKTOP_WS_URL = backendWsUrl;
