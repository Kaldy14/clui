...
> {
  return Effect.gen(function* () {
    const remoteDir = yield* makeTempDir("clui-git-remote-");
...
function createTextGeneration(overrides: Partial<FakeGitTextGeneration> = {}): TextGenerationShape {
  ...
  return {
    ...
    generateThreadTitle: () => Effect.succeed({ title: "Test Title" }),
  };
}
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  it.effect("status includes PR metadata when branch already has an open PR", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("clui-git-manager-");
      ...
      const { manager } = yield* makeManager({
        ghScenario: {
          pullRequest: {
            ...
            url: "https://github.com/pingdotgg/clui/pull/642",
            baseRefName: "main",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("clui/pr-91/main");
        expect(result.worktreePath).not.toBeNull();
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("clui/pr-92/main");
