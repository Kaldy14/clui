...
const BASH_PATTERNS: ReadonlyArray<readonly [RegExp, AgentActivityStatus]> = [
  ...
  [
    /\b(?:vitest|jest|pytest|go\s+test|cargo\s+test|swift\s+test|xcodebuild\b[\s\S]*\btest)\b/u,
    "testing",
  ],
  ...
  [
    /\b(?:vite\s+build|next\s+build|webpack\b|rollup\b|esbuild\b|cargo\s+build|go\s+build)\b/u,
    "building",
  ],
